...
public class backgroundLoop : MonoBehaviour
{
    ...
    void loadChildObjects(GameObject obj)
    {
        ...
        for (int i = 0; i <= childsNeeded; i++)
        {
            ...
            c.transform.position = new Vector3(obj.transform.position.x, objectWidth * i, 0f);
            c.name = obj.name + i;
        }
        Destroy(clone);
    ...
    void repositionChildObjects(GameObject obj)
    {
        ...
        if (children.Length > 1)
        {
            ...
            if (transform.position.y + screenBounds.y > lastChild.transform.position.y + halfObjectWidth)
            {
                ...
                firstChild.transform.position = new Vector3(lastChild.transform.position.x, lastChild.transform.position.y + halfObjectWidth * 2, 0f);
            }
            else if (transform.position.y - screenBounds.y < firstChild.transform.position.y - halfObjectWidth)
            {
                lastChild.transform.SetAsFirstSibling();
                lastChild.transform.position = new Vector3(firstChild.transform.position.x, firstChild.transform.position.y - halfObjectWidth * 2, 0f);
            }
        }
    }
    void Update()
